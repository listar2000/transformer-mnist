...
def validate_image_transformer(val_loader, model, loss_fn, device):
    with torch.no_grad():
        total_samples = 0
        correct_samples = 0
        total_loss = 0
...
def train_image_transformer(epochs=10, batch_size=64, device='gpu'):
    ...
    training_data, validation_data, test_data = read_mnist(datapath="./data")
    ...
    for epoch in range(epochs):
        ...
        # start validating
        validate_image_transformer(val_loader, model, loss_fn, device)

    return model


if __name__ == "__main__":
    trans_model = train_image_transformer(10, 64, "GPU")
